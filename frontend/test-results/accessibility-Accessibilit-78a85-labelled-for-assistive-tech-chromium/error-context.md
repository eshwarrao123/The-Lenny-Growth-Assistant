...
- Name: accessibility.spec.ts >> Accessibility >> version switcher is labelled for assistive tech
- Location: tests\e2e\accessibility.spec.ts:67:7
...
Locator: getByRole('region', { name: 'Artifact viewer' }).getByRole('group', { name: 'Artifact version' })
Expected: visible
Timeout: 10000ms
Error: element(s) not found

Call log:
  - Expect "toBeVisible" getByRole('region', { name: 'Artifact viewer' }).getByRole('group', { name: 'Artifact version' }) with timeout 10000ms
  - waiting for getByRole('region', { name: 'Artifact viewer' }).getByRole('group', { name: 'Artifact version' })
...
```ts
  ...
  3   |   HTML_ARTIFACT,
  4   |   MARKDOWN_ARTIFACT,
  5   |   MARKDOWN_ARTIFACT_V2,
  6   |   MOCK_ARTIFACT_ID,
  7   |   MOCK_ARTIFACT_V2_ID,
  8   |   artifactPayload,
  9   |   htmlArtifactEvents,
  10  |   markdownArtifactEvents,
  11  |   mockArtifactVersions,
  12  |   mockChatStream,
  13  |   sendMessage,
  14  |   startNewChat,
  15  | } from './helpers'
  16  | 
  17  | test.describe('Accessibility', () => {
  18  |   test('artifact controls expose useful accessible names', async ({ page }) => {
  19  |     await startNewChat(page)
  20  |     await mockChatStream(page, markdownArtifactEvents(MOCK_ARTIFACT_ID))
  21  | 
  22  |     await sendMessage(page, 'Create a markdown checklist')
  23  | 
  24  |     const viewer = page.getByRole('region', { name: 'Artifact viewer' })
